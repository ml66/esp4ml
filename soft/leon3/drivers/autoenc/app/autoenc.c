...
void *accelerator_thread( void *ptr )
{
	...
	gettime(&th_start);
	rc = ioctl(info->fd, AUTOENC_IOC_ACCESS, info->desc);
	gettime(&th_end);
	if(rc < 0) {
		perror("ioctl");
		exit(1);
...
static void prepare_autoenc(int *fd, contig_handle_t *mem, accelerator_thread_info_t *info, struct autoenc_access *desc, const char *device_name, unsigned id)
{
...
int main(int argc, char *argv[])
{
	...
	struct autoenc_access desc[3];

	void *rp;

	printf("=== Point-to-point test w/ autoenc accelerator ===\n");
	...
	for (i = 0; i < 3; i++) {
		sprintf(device_name, "/dev/autoenc.%d", i);
		prepare_autoenc(&fd[i], &hwbuf, &info[i], &desc[i], device_name, i);
	}
