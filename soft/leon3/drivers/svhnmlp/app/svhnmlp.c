...
void *accelerator_thread( void *ptr )
{
	...
	gettime(&th_start);
	rc = ioctl(info->fd, SVHNMLP_IOC_ACCESS, info->desc);
	gettime(&th_end);
	if(rc < 0) {
		perror("ioctl");
		exit(1);
...
static void prepare_svhnmlp(int *fd, contig_handle_t *mem, accelerator_thread_info_t *info, struct svhnmlp_access *desc, const char *device_name, unsigned id)
{
...
int main(int argc, char *argv[])
{
	...
	struct svhnmlp_access desc[3];

	void *rp;

	printf("=== Point-to-point test w/ svhnmlp accelerator ===\n");
	...
	for (i = 0; i < 3; i++) {
		sprintf(device_name, "/dev/svhnmlp.%d", i);
		prepare_svhnmlp(&fd[i], &hwbuf, &info[i], &desc[i], device_name, i);
	}
