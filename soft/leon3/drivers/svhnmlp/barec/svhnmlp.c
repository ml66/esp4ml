/**
 * Baremetal device driver for SVHNMLP
 *
...
#define SLD_SVHNMLP   0x43
#define DEV_NAME "sld,svhnmlp"
...
#define SVHNMLP_INBUF_SIZE (SIZE_IN * sizeof(token_t))
#define SVHNMLP_OUTBUF_SIZE (SIZE_OUT * sizeof(token_t))
#define SVHNMLP_BUF_SIZE (SVHNMLP_INBUF_SIZE + SVHNMLP_OUTBUF_SIZE)

/* Size of the contiguous chunks for scatter/gather */
#define CHUNK_SHIFT 13
#define CHUNK_SIZE BIT(CHUNK_SHIFT)
#define NCHUNK ((SVHNMLP_BUF_SIZE % CHUNK_SIZE == 0) ?			\
			(SVHNMLP_BUF_SIZE / CHUNK_SIZE) :			\
			(SVHNMLP_BUF_SIZE / CHUNK_SIZE) + 1)

// User defined registers
#define SVHNMLP_NINPUTS_REG 0x40

static int validate_svhnmlp(token_t *mem)
{
    int i, j;
    int rtn = 0;
    int base;
...
int main(int argc, char * argv[])
{
	...
	ndev = probe(&espdevs, SLD_SVHNMLP, DEV_NAME);
	...
	// Allocate memory
	mem = aligned_malloc(SVHNMLP_BUF_SIZE);
#ifndef __riscv
	...
	iowrite32(dev, DST_OFFSET_REG, SVHNMLP_INBUF_SIZE);
	iowrite32(dev, SVHNMLP_NINPUTS_REG, NINPUTS);
	...
	/* Validation */
	printf("  validating...\n");
	errors = validate_svhnmlp(&mem[SIZE_IN]);
	if (errors)
		printf("  ... FAIL\n");
	else
		printf("  ... PASS\n");
